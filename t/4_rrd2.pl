...
subtest 'bigint' => sub {
    {
        ...
        for my $tms( qw/ 2014-09-16T22:55:00 
            ...
            2014-09-16T23:05:00 
            2014-09-16T23:10:00 
            2014-09-16T23:15:00 2014-09-16T23:20:00 2014-09-16T23:25:00 /) {

            $buf .= sprintf("%s %u %u %u %u\n", $tms, $val32, $val32, $val64, $val64 );
            $val32 += 5 * 300;
            if ($val32 > 2 ** 32) {
                $val32 -= 2 ** 32;
            }
        ...
        my $cmd = "rrdtool fetch $FindBin::Bin/rrd/storage/bigint.rrd AVERAGE -r 300 -s 1410875700 -e 1410877200";
        print $cmd . "\n";
        my $buf = `$cmd`;
        print $buf;
        ok ($buf=~/1410876000: \d/);

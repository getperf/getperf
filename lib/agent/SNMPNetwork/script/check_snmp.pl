...
my $USAGE     = "check_snmp, version : 2.5.0(UNIX)\n" .
	"Usage : check_snmp.pl [--silent] [--community=...] [--version=[1|2c|3]] [--networks=...]\n";

GetOptions('--odir=s' => \$ODIR, '--help' => \$SHOWVER, '--version=s' => \$SNMP_VER, 
	'--community=s' => \$COMMUNITY, '--networks=s' => \$NETWORKS, '--silent' => \$SILENT)
	 || die $USAGE;
...
sub verifyNetInterface {
	...
	for my $host(keys %{$$ref_snmp->{hosts}}) {
		...
		for my $oid('ifOperStatus', 'ifDescr', 'sysName') {
			my $test = "${oid}_${host}";
...
sub reportNetInterface {
	...
	for my $host_id(sort keys %{$$ref_snmp->{$oid}}) {
		...
		if ($rc == 0) {
			...
			while (<IN>) {
				chomp;
				# SNMPv2-MIB::sysName.0 = STRING: CL-NY2-A0203-LEAFPX02
				if ($_=~/sysName\.(\d+) = STRING: (.+)$/) {
					$device{$host_id}{sysName} = $2;
					# print "CHECK $host_id = $2\n";
				}
			}
		}
	}
	...
	for my $host_id(sort keys %device) {
		...
		$buf .= "    sysName: $device{$host_id}{sysName}\n";

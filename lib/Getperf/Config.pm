...

{
	...
	sub new {
	    ...
	    my $self = bless {
			...
			ssl_root_ca        => $ENV{GETPERF_SSL_COMMON_NAME_ROOT_CA} || $config->{GETPERF_SSL_COMMON_NAME_ROOT_CA} || 'getperf_ca',
			ssl_inter_ca       => $ENV{GETPERF_SSL_COMMON_NAME_INTER_CA} || $config->{GETPERF_SSL_COMMON_NAME_INTER_CA} || 'getperf_inter',
			license_policy     => $config->{GETPERF_LICENSE_POLICY} || 'none',
			ws_tomcat_owner    => $config->{GETPERF_WS_TOMCAT_OWNER} || 'pscommon',
			ws_server_name     => $ENV{GETPERF_WS_SERVER_NAME} || $config->{GETPERF_WS_SERVER_NAME} || '127.0.0.1',
			...
			ws_admin_server    => $ENV{GETPERF_WS_ADMIN_SERVER} || $config->{GETPERF_WS_ADMIN_SERVER} || '127.0.0.1',
			...
			ws_data_server     => $ENV{GETPERF_WS_DATA_SERVER} || $config->{GETPERF_WS_DATA_SERVER} || '127.0.0.1',

package Getperf::Command::Site::Oracle::SpOraSga;
use strict;
use warnings;
use Data::Dumper;
use Time::Piece;
...
sub parse {
	...
	while (my $line = <IN>) {
		...
		$mem_area =~s/[\$\s]+/_/g;
		$mem_area =~s/[\(\)]//g;
		next if ($mem_area eq 'pga_target');
	...
	for my $instance(keys %results) {
		...
		for my $mem_area(keys %{$results{$instance}}) {
			my $output = "Oracle/${host_suffix}/device/ora_sga__${mem_area}.txt";
			$data_info->regist_device($host_suffix, 'Oracle', "ora_sga", 

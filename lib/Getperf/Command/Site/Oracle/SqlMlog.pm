package Getperf::Command::Site::Oracle::SqlMlog;
use strict;
use warnings;
use Data::Dumper;
use Time::Piece;
...
sub parse {
	...
	my $sec  = $data_info->start_time_sec->epoch + 600;
	if (!$sec) {
		return;
	}
	...
	for my $instance(keys %{$results}) {
		$data_info->regist_metric($instance, 'Oracle', 'sql_mlog', \@headers);
		my $output = "Oracle/${instance}/sql_mlog.txt";

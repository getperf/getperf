...
sub parse {
    ...
    # Report "Time Model"
    {
        ...
        for my $instance (keys %{$events}) {
            my $host_suffix = "${host}_${instance}";
            my $output  = "Oracle/${host_suffix}/oraTemp_time_model_rac.txt";
            my %data    = ($sec => $events->{$instance});
            # print Dumper $header;
            $data_info->regist_metric($host_suffix, 'Oracle', 'oraTemp_time_model_rac', $header);
            $data_info->simple_report($output, \%data, $header);
        }
    }
    # Report "Foreground Wait Classes"
    {
        ...
        for my $instance (keys %{$events}) {
            my $host_suffix = "${host}_${instance}";
            my $output  = "Oracle/${host_suffix}/oraTemp_foreground_wait_rac.txt";
            my %data    = ($sec => $events->{$instance});
            # print Dumper $header;
            $data_info->regist_metric($host_suffix, 'Oracle', 'oraTemp_foreground_wait_rac', $header);
            $data_info->simple_report($output, \%data, $header);
        }
    }
    ...
    # Report "Top Timed Events"
    {
        ...
        for my $instance (keys %{$events}) {
            ...
            for my $device (keys %{$header_keys}) {
                my $output  = "Oracle/${host_suffix}/device/oraTemp_event_rac2__${device}.txt";
                my $value = 0;
                ...
                $data_info->regist_device($host_suffix, 'Oracle', 'oraTemp_event_rac2', 
        ...
        #         my $output  = "Oracle/${host_suffix}/device/oraTemp_ping_rac__${device}.txt";
        #         # print "$output\n";
        #         my %data    = ($sec => $events->{$instance}{$device});
        #         # print Dumper $header;
        #         $data_info->regist_device($host_suffix, 'Oracle', 'oraTemp_ping_rac', 
    ...
    # Report "Top Timed Background Events"
    {
        ...
        for my $instance (keys %{$events}) {
            my $host_suffix = "${host}_${instance}";
            for my $device (keys %{$header_keys}) {
                my $output  = "Oracle/${host_suffix}/device/oraTemp_bgevent_rac2__${device}.txt";
                my $value = 0;
                if ($events->{$instance}{$device}) {
                    $value = $events->{$instance}{$device};
                }
                ...
                $data_info->regist_device($host_suffix, 'Oracle', 'oraTemp_bgevent_rac2', 
                                          $device, $device_text, ['elapse']);
                $data_info->simple_report($output, \%data, ['elapse']);
            }
        }
    ...
    # Report "System Statistics - Per Second"
    {
        ...
        for my $instance (keys %{$events}) {
            my $host_suffix = "${host}_${instance}";
            my $output  = "Oracle/${host_suffix}/oraTemp_system_stat_rac.txt";
            my %data    = ($sec => $events->{$instance});
            # print Dumper $header;
            $data_info->regist_metric($host_suffix, 'Oracle', 'oraTemp_system_stat_rac', $header);
            $data_info->simple_report($output, \%data, $header);
        }
    }
    ...
    # Report "Global Cache Efficiency Percentages" : CacheEfficiency
    {
        ...
        for my $instance (keys %{$events}) {
            my $host_suffix = "${host}_${instance}";
            my $output  = "Oracle/${host_suffix}/oraTemp_cache_hit_rac.txt";
            my %data    = ($sec => $events->{$instance});
            # print Dumper $header;
            $data_info->regist_metric($host_suffix, 'Oracle', 'oraTemp_cache_hit_rac', $header);
            $data_info->simple_report($output, \%data, $header);
        }
    }
    ...
    # Report "Interconnect Client Statistics (per Second)" : InterconnectTraffic
    {
        ...
        for my $instance (keys %{$events}) {
            my $host_suffix = "${host}_${instance}";
            my $output  = "Oracle/${host_suffix}/oraTemp_interconnect_rac.txt";
            my %data    = ($sec => $events->{$instance});
            # print Dumper $header;
            $data_info->regist_metric($host_suffix, 'Oracle', 'oraTemp_interconnect_rac', $header);
            $data_info->simple_report($output, \%data, $header);
        }
    }
	return 1;

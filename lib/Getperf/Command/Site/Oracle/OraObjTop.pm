...
# 2016/06/08 23:05:02,RTDMGR,RT_FLOW_INFO,TABLE,DEFAULT,4421844992,1101222016,29093051
# 2016/06/08 23:05:02,RTDMGR,RT_BFX_FOUPREQ_STATE_HIST,TABLE,DEFAULT,3984588800,210043120,2428369
# 2016/06/08 23:05:02,RTDMGR,RM_KATSERVICE_LOG,TABLE,DEFAULT,2884632576,2342720,725422
# 2016/06/08 23:05:02,RTDMGR,XAM_RT_MAINJOB_LOT(XAM_RT_MAINJOB_LOT_20160528),TABLE PARTITION,DEFAULT,104857600,612480,611610
# 2016/06/08 23:05:02,RTDMGR,XAM_RT_MAINJOB_LOT(XAM_RT_MAINJOB_LOT_20160525),TABLE PARTITION,DEFAULT,104857600,611552,610682
# 2016/06/08 23:05:02,RTDMGR,XAM_RT_MAINJOB_LOT(XAM_RT_MAINJOB_LOT_20160603
...
sub parse {
	...
	while (my $line = <$in>) {
		...
		my ($timestamp, $owner, $segment_name, $object_type, $buffer_type, @values) = split(/\s*[,\|]\s*/, $line);
		...
		for my $col (0..2) {
			...
			my $value = $values[$col];
			$results{$object_key}{$sec}{$header} = $value;
			$obj_stats{$object_key}{$header}    += $value;
		}
	}
	close($in);
	my $query_items =
	...
	my %registered_obj_key = ();
	for my $sort_key(qw/buffer_gets disk_reads/) {
		my $metric = 'ora_obj_top_by_' . $sort_key;
		my @obj_ranks = sort { $obj_stats{$b}{$sort_key} <=> $obj_stats{$a}{$sort_key} } keys %obj_stats;
		my $rank = 1;
		for my $obj_key(@obj_ranks) {
			next if (defined($registered_obj_key{$obj_key}));

package Getperf::Command::Site::Oracle::SpOraSharedPool;
use strict;
use warnings;
use Data::Dumper;
use Time::Piece;
...
sub parse {
	...
	while (my $line = <IN>) {
		...
		# print $line . "\n";
		next if (scalar(@csv) != 4 ||  $csv[0] =~/(TIME|DATE)/);
	...
	my @metrics = qw/db_block_hash_buckets kglh0 kglhd free_memory sqla/;
	for my $instance(keys %results) {
        my $host_suffix = $host;
        $host_suffix = "${host}_${instance}" if ($instance > 1);
        for my $mem_area(@metrics) {
	    ...
	    $data_info->regist_devices_alias($host_suffix, 'Oracle', 'ora_shared_pool', 
	        'ora_shared_pool_main', \@metrics, undef);
	}
	return 1;
}

	...
	for my $infile(@infiles) {
		...
		while (<IN>) {
			...
			# データの抽出
				...
				my @vals =split(/,/, $line);
				# 簡易フォーマットチェック
#				if (scalar(@vals) != 5 && scalar(@vals) != 4) {
#					next;
#				} 
				# PIDをキーにハッシュ作成
				my $sesid = shift(@vals);
...
#	for my $key(sort keys %sesstat) {
#		my $line = join(",", ($key , $sesstat{$key}));
#		print $line . "\n";
#	}
}
...
sub check_proc {
	...
	if ($ORASES{$pid}) {
		...
		$schema = 'NULL' if ($schema eq '');	# ユーザがnullの場合
		my $usercat = 'ETC';
		for my $key(@USRCAT) {
			if ($schema=~/$key/) {
				$usercat = $key;
...
sub mklogfile_etc {
	...
	my $dtcnt = scalar(keys %DATELIST);

	# ファイルオープン
	my $outfile = "$ODIR/$fname";
	print("MKLOG : $outfile $dtcnt\n");
	open(OUT, ">$outfile") || die "Can't open $outfile : $1";

	# ヘッダ出力
	my $header = 'ITEM     CPU    ';
	print OUT $header . "\n";
	for my $key(sort {$dat{$b}<=>$dat{$a}} keys %dat) {
		my $line = sprintf("%40s %8.2d", $key, $dat{$key}  * 100.0 / $INTERVAL / $dtcnt);
		print OUT $line . "\n";
	}
	close(OUT);
}
...
sub mkorapslog {
	...
	my (%pscpu_host_etc, %pscpu_prog_etc, %pscpu_usr_etc);
	my ($sec);
	my $cnt = 0;
	...
	while (<IN>) {
		...
		if ($cnt > 1) {
			my (undef, $schema, $host, $prog) = split(/,/, $ORASES{$pid});
			print OUT join(",", 
			...
			if ($orahost eq 'ETC') {
				$pscpu_host_etc{$host} += $cpusec * 1.0;
			}
			# 日付、ユーザ名でCPU時間集計
			my $key = $dt . ',' . $orausr;
			$pscpu_usr{$key} += $cpusec * 1.0;
			if ($orausr eq 'ETC') {
				$pscpu_usr_etc{$schema} += $cpusec * 1.0;
			}
			# 日付、Programカテゴリ名でCPU時間集計
			my $key = $dt . ',' . $oracat;
			$pscpu_prog{$key} += $cpusec * 1.0;
			if ($oracat eq 'ETC') {
				$pscpu_prog_etc{$prog} += $cpusec * 1.0;
			}
		}
	}
	print "$cnt\n";
	...
	mklogfile('orapshost_ura.txt', $HOSTCAT_STR, %pscpu_host);
	mklogfile('orapsusr_ura.txt', $USRCAT_STR, %pscpu_usr);
	mklogfile('orapsprog_ura.txt', $PROGCAT_STR, %pscpu_prog);
	mklogfile_etc('oraetcps_host_ura.txt', %pscpu_host_etc);
	mklogfile_etc('oraetcps_usr_ura.txt',  %pscpu_usr_etc);
	mklogfile_etc('oraetcps_prog_ura.txt', %pscpu_prog_etc);
}

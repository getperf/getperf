package Getperf::Command::Site::Db2::Db2SqlRankGraph;
use strict;
use warnings;
use Data::Dumper;
use Time::Piece;
...
use Getperf::Command::Site::Db2::Db2SqlRankGraphDaily;
...
sub parse {
	...
	my %graph_headers = (
		"NUM_EXEC"        => "SQL Exec Ranking",
		"EXEC_TIME"       => "SQL Exec Time Ranking",
		"CPU_TIME"        => "SQL CPU Time Ranking",
		"ACT_WAIT_TIME"   => "SQL Active Wait Time Ranking",
		"LOCK_WAIT_TIME"  => "SQL Lock Wait Time Ranking",
		"LOCK_WAITS"      => "SQL Lock Wait Ranking",
		"DISK_READ_TIME"  => "Disk Read Time Ranking",
		"DISK_WRITE_TIME" => "Disk Write Time Ranking",
		"ROWS_MODIFIED"   => "Modified Rows Ranking",
		"ROWS_READ"       => "Read Rows Ranking",
		"FED_WAIT_TIME"   => "Federation Time Ranking",
	);
	...
	my $query = "select `stmtid`, `metric`, sum(`value`) ". 
				" from `db2_sql_rank` where `clock` = $sec ".
	...
	for my $header(@rrd_headers2) {
		...
		# sql_stat->{$header};
		# DIRECT_READ_TIME, DIRECT_WRITE_TIME は除外設定する

		if ($header=~/(DISK_READ_TIME|DISK_WRITE_TIME)/) {
			print "SKIP!!!!!!!!!!!!!! DISK_READ_TIME, DISK_WRITE_TIME\n";
			next;
		}
		...
		for my $stmtid(@sql_ranks) {
			...
			$data_info->regist_device($host, 'Db2', 
				'db2_sql_top', $stmtid, undef, \@rrd_headers2);
			my $output = "Db2/${host}/device/db2_sql_top__${stmtid}.txt";
			my $results;
			$results->{$sec} = $sql_stats->{$stmtid};
			warn "RANK : $header, $rank, $stmtid, " . $sql_stats->{$stmtid}{$header} . "\n";
			$data_info->pivot_report($output, $results, \@rrd_headers2);
	...
	my $report_file = file($data_info->absolute_summary_dir, "report__${host}.txt");
	...
	# 00:00:00 台の実行の場合は Daily, Weekly 集計をする
	my $sumup_average_debug = 0;
	if ($sumup_average_debug || $data_info->{summary_dir} =~/\/000000/) {
		print "Run Daily Weekly Rankig Summary \n";
		my $test2 = Getperf::Command::Site::Db2::Db2SqlRankGraphDaily->new;
		$test2->parse($data_info);
	}

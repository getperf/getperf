package Getperf::Command::Site::Db2::Db2LogIo;
use strict;
use warnings;
use Data::Dumper;
use Time::Piece;
...
sub parse {
    ...
    my $results;
    my $step = 600;
    my @headers = qw/logWrites numLogWriteIo/;
    ...
    my $defalutDat = {$sec => "0 0"};
    duplicateDat($defalutDat, $sec, "0 0");
    my @databases = db_list(0);
    for my $db(@databases) {
    # for my $service(keys %{$results}) {
        my $service = "${site}-${db}";
        print $service. "\n"; 
        $data_info->regist_metric($service, 'Db2', 'db2_log_io', \@headers);
        my $output = "Db2/${service}/db2_log_io.txt";

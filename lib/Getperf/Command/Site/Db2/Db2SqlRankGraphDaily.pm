package Getperf::Command::Site::Db2::Db2SqlRankGraphDaily;
use strict;
use warnings;
use Data::Dumper;
use Time::Piece;
...
use Getperf::Command::Site::Db2::Db2SqlRankGraphWeekly;
use Storable;
...
sub parse {
	...
	my %graph_headers = (
		# "NUM_EXEC"        => "Daily SQL Exec Ranking",
		"EXEC_TIME"       => "Daily SQL Exec Average Time Ranking",
		"CPU_TIME"        => "Daily SQL CPU Average Time Ranking",
		# "ACT_WAIT_TIME"   => "Daily SQL Active Wait Time Ranking",
		# "LOCK_WAIT_TIME"  => "Daily SQL Lock Wait Time Ranking",
		# "LOCK_WAITS"      => "Daily SQL Lock Wait Ranking",
		# "DISK_READ_TIME"  => "Daily Disk Read Time Ranking",
		# "DISK_WRITE_TIME" => "Daily Disk Write Time Ranking",
		# "ROWS_MODIFIED"   => "Daily Modified Rows Ranking",
		# "ROWS_READ"       => "Daily Read Rows Ranking",
		# "FED_WAIT_TIME"   => "Daily Federation Time Ranking",
	);
...
#				" from `db2_sql_rank` where `clock` > unix_timestamp() - 24*3600 ".
#				" and `member` like \"$host%\" " .
#                " and `metric` in ('TOTAL_CPU_TIME', 'STMT_EXEC_TIME') " .
#				" and `stmt_type_id` like 'DML%' " .
#				" group by `stmtid`, `metric`";

	my $query = "select `stmtid`, `metric`, sum(`sum_value`) / (24*6) ".
                " from `db2_sql_rank_daily` where `clock` > unix_timestamp() - 24*3600  ".
                " and `member` like \"$host%\"  ".
                " and `metric` in ('TOTAL_CPU_TIME', 'STMT_EXEC_TIME') ".
                " group by `stmtid`, `metric` ";
	...
	my $test2 = Getperf::Command::Site::Db2::Db2SqlRankGraphWeekly->new;
	$test2->parse($data_info);

	# $self->purge_sql_hash_rrd_data($data_info, $host);
	# print Dumper $reports;
	my $report_file = file($data_info->absolute_storage_dir, "report_daily__${host}.txt");

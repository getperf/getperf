package Getperf::Command::Site::Db2::Db2LogBusy;
use strict;
use warnings;
use Data::Dumper;
use Time::Piece;
...
sub parse {
    ...
    my $results;
    my $step = 600;
    my @headers = qw/logWrites logWriteTime numLogWriteIo/;
    ...
    while (my $line = <$in>) {
        ...
        # $results->{"${site}-${db}-${member}"}{$sec} = join(" ", @columns);
        my $dat = join(" ", @columns);
    ...
    # for my $service(keys %{$results}) {
        ...
        $data_info->regist_metric($service, 'Db2', 'db2_log_busy', \@headers);
        my $output = "Db2/${service}/db2_log_busy.txt";

package Getperf::Command::Site::Db2Hourly::TryInstantCfWaitTime;
use strict;
use warnings;
use Data::Dumper;
use Time::Piece;
...
'TOTAL_CF_WAIT_TIME_MICRO' ,'CF_WAIT_TIME_MICRO',
);
...
sub parse {
	...
	for my $cfhost(keys %results) {
		...
		$data_info->regist_metric($cfhost, 'Db2', 'try_instant_cf_wait_time', \@headers);
		my $output = "Db2/${cfhost}/try_instant_cf_wait_time.txt";	# Remote collection
		$data_info->pivot_report($output, $cfresults, \@headers);
	}
	my $options = {'enable_first_load' => 1, 'regist_abs' => 1};
	db2_update_stats($data_info, $host, 'mon_get_cf_wait_time_tryinstant', $stats, $options);
	return 1;
}

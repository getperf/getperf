...
* 集計定義
   ...
   エージェントからの受信データのファイル名から該当する集計スクリプトを検索します。検索されたスクリプトで、データの集計、時系列データベースへのロード、ノード定義、ビュー定義の更新をします。ノード定義、ビュー定義はCacti, Zabbix の設定で使用します。スクリプトは専用API :doc:`./04_AggregateScriptAPI` を用いてコーディングします。

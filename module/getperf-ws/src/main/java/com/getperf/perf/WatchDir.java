...
public class WatchDir {
    ...
    private void register(Path dir) throws IOException {
        ...
        if (trace) {
            ...
            if (prev == null) {
                LOG.info("Register directory watch : {}", dir);
            } else {
                if (!dir.equals(prev)) {
                    LOG.info("Update directory watch : {} -> {}", prev, dir);
                }
            }
        }
        keys.put(key, dir);
    ...
    WatchDir(Path dir, boolean recursive) throws IOException {
        ...
        if (recursive) {
            LOG.info("Scanning directory watch : {}", dir);
            registerAll(dir);
            LOG.info("Done.");
        } else {
            register(dir);
    ...
    void processEvents() {
        for (;;) {
            LOG.info("Catch directory update events");
            ...
            for (WatchEvent<?> event: key.pollEvents()) {
                ...
                if (kind == ENTRY_MODIFY) {
                    synchronized (SiteConfig.class) {
                        ...
                        boolean put_ok = repos.putSiteInfo(child.toFile());
                        LOG.info("{}: {}: {}", event.kind().name(), child, put_ok);
